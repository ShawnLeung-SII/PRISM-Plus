#!/bin/bash
# PRISM+ Stage 1 v0.6.0 — C1 + v9-style loss
set -u
...
LOG_DIR="$CACHE_ROOT/0-XIUJIANLIANG/checkpoints/prism_plus/stage1_v6/logs"
mkdir -p "$LOG_DIR"
LOGFILE="$LOG_DIR/train_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "$LOGFILE") 2>&1

echo "========== PRISM+ Stage 1 v0.6.0 — C1 + v9-style loss =========="
echo "Date: $(date)"
echo "Host: $(hostname)"
echo "Log:  $LOGFILE"
...
torchrun --nproc_per_node="$GPU_COUNT" --master_port=29500 \
    tools/train_bnd_v6.py \
    --config configs/stage1_v6.yaml \
    --seed 42
